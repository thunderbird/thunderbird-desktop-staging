...
function test_tab_reorder_tabbar() {
  ...
  assert_equals(
    tab1,
    mc.tabmail.tabContainer.childNodes[3],
    "Moving tab1 failed"
  );
  switch_tab(3);
  assert_selected_and_displayed(msgHdrsInFolder[0]);
  ...
  assert_true(
    tab3 == mc.tabmail.tabContainer.childNodes[2],
    "Moving tab3 failed"
  );
  switch_tab(2);
...
function test_tab_reorder_window() {
  ...
  assert_true(
    mc.tabmail.tabContainer.childNodes.length == 1,
    "Moving tab to new window failed, tab still in old window"
  );

  assert_true(
    mc2.tabmail.tabContainer.childNodes.length == 2,
