...
// Wrap in a block to prevent leaking to window scope.
{
  ...
  class CalendarListTree extends customElements.get("tree") {
        ...
        switch (event.key) {
          ...
          case " ": {
            if (this.tree.currentIndex > -1) {
              const col = this.tree.columns.getColumnFor(this.querySelector(".checkbox-treecol"));
          ...
          case "ArrowDown": {
            ...
            if (
              (event.key == "ArrowUp" && index < this.mCalendarList.length - 1) ||
    ...
    get sheet() {
      if (!this.mCachedSheet) {
        ...
        if (!this.mCachedSheet) {
          cal.ERROR(
            "Could not find calendar-management.css, needs to be added to " +
    ...
    set compositeCalendar(compositeCal) {
      if (this.mCompositeCalendar) {
        throw Components.Exception(
          "A composite calendar has already been set",
    ...
    addCalendar(calendar) {
      ...
      if (!composite.defaultCalendar || calendar.id == composite.defaultCalendar.id) {
    ...
    removeCalendar(calendar) {
      ...
      for (let i = 0; i < sheet.cssRules.length; i++) {
        if (
          sheet.cssRules[i] == this.ruleCache[calendar.id][0] ||
    ...
    updateCalendarColor(calendar) {
      ...
      if (!(calendar.id in this.ruleCache)) {
        const ruleString =
          ".calendar-list-tree > treechildren" +
          "::-moz-tree-cell(color-treecol, id-" +
          calendar.id +
          ") {}";

        const disabledRuleString =
          ".calendar-list-tree > treechildren" +
          "::-moz-tree-cell(color-treecol, id-" +
          calendar.id +
          ", disabled) {}";
        ...
        try {
          ...
          sheet.ownerNode.addEventListener("load", () => this.updateCalendarColor(calendar), {
            once: true,
          });
          return;
        }
    ...
    getCalendarFromEvent(event, col = {}, row = {}) {
      if (event.clientX && event.clientY) {
        ...
        const { col: cellCol, row: cellRow } = this.tree.getCellAt(event.clientX, event.clientY);
    ...
    getRowProperties(row) {
      ...
      const disabledState =
        !isDisabled || (isDisabled && this.disabledState == "ignore") ? "enabled" : "disabled";
    ...
    foreignDrop(event) {
      const hasDropped =
        event.clientY < this.tree.getBoundingClientRect().y
  ...
  class CalendarListTreeFull extends CalendarListTree {
    connectedCallback() {
      ...
      this.calMgrObserver = {
        ...
        onCalendarRegistered(calendar) {
          ...
          const inComposite = calendar.getProperty(composite.prefPrefix + "-in-composite");
    ...
    get compositeCalendar() {
      if (!this.mCompositeCalendar) {
        throw Components.Exception(
          "Calendar list has no composite calendar yet",
