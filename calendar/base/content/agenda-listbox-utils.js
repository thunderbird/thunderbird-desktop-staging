...
agendaListbox.init = async function() {
  ...
  if (!("getCheckbox" in showTodayHeader)) {
    await new Promise(resolve =>
      showTodayHeader.addEventListener("bindingattached", resolve, { once: true })
    );
  }
  if (!("getCheckbox" in showTomorrowHeader)) {
    await new Promise(resolve =>
      showTomorrowHeader.addEventListener("bindingattached", resolve, { once: true })
    );
  }
  if (!("getCheckbox" in showSoonHeader)) {
    await new Promise(resolve =>
      showSoonHeader.addEventListener("bindingattached", resolve, { once: true })
    );
  }
